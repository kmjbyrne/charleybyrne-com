...
const postPath = computed(() => post.value?.path ?? `/blog/${slug}`);
...
const { data: allTags } = await useAsyncData("all-tags", async () => {
  const posts = await usePublicPosts(queryCollection("posts"))
    .select("tags")
    .all();
...
const currentIndex = computed(
  () => allPosts.value?.findIndex((p) => p.path === postPath.value) ?? -1,
);
...
const recentPosts = computed(
  () =>
    allPosts.value?.filter((p) => p.path !== postPath.value).slice(0, 4) ?? [],
);
      ...
      <div class="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_16rem]">
        <article class="min-w-0 py-12">
          <div v-if="post.image" class="mb-10 max-w-2xl">
            ...
            />
            <p
              v-if="post.imageCredit"
              class="text-dimmed mt-2 text-xs text-right"
            >
              Photo:
              <a
          ...
          <header class="mb-10 max-w-2xl">
            <div class="mb-4 flex flex-wrap items-center gap-2">
              ...
              <span v-if="readingTime" class="text-dimmed text-sm">{{
                readingTime
              }}</span>
            </div>
            <h1
              class="text-highlighted mb-4 text-3xl font-semibold leading-tight tracking-tight sm:text-4xl"
            >
              {{ post.title }}
            </h1>
            <p
          ...
          <div
            ...
            class="mt-16 max-w-2xl border-t border-default pt-10"
          >
            <p
              class="text-highlighted mb-6 text-xs font-semibold uppercase tracking-widest"
            >
              Related Posts
            </p>
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
              <NuxtLink
                ...
                :key="r.path"
                :to="useFlatBlogPath(r.path)"
                class="group flex flex-col gap-2"
              >
                <p
                  class="text-highlighted text-sm font-semibold leading-snug transition-colors group-hover:text-primary"
                >
                  {{ r.title }}
                </p>
          ...
          <div class="mt-10 max-w-2xl border-t border-default pt-8">
            <div class="flex items-center justify-between gap-4">
              <NuxtLink
                v-if="prevPost"
                :to="useFlatBlogPath(prevPost.path)"
              ...
              >
                ...
                <span
                  class="text-muted group-hover:text-highlighted text-sm font-medium leading-snug transition-colors"
                >
                  {{ prevPost.title }}
                </span>
              </NuxtLink>
              ...
              <NuxtLink
                v-if="nextPost"
                :to="useFlatBlogPath(nextPost.path)"
              ...
              >
                ...
                <span
                  class="text-muted group-hover:text-highlighted text-sm font-medium leading-snug text-right transition-colors"
                >
                  {{ nextPost.title }}
                </span>
              </NuxtLink>
        ...
        <aside class="hidden lg:block py-12">
          <div
            class="sticky top-[calc(var(--ui-header-height)+2rem)] flex flex-col gap-8"
          >
            <UContentToc
              v-if="post.body?.toc?.links?.length"
              highlight
            ...
            <div v-if="recentPosts.length" class="flex flex-col gap-3">
              <p
                class="text-highlighted text-xs font-semibold uppercase tracking-widest"
              >
                Recent Articles
              </p>
              <div class="flex flex-col gap-4">
                ...
                >
                  <span
                    class="text-muted group-hover:text-highlighted text-sm leading-snug transition-colors"
                  >
                    {{ r.title }}
                  </span>
            ...
            <div v-if="allTags?.length" class="flex flex-col gap-3">
              <p
                class="text-highlighted flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest"
              >
              ...
              <div class="flex flex-wrap gap-2">
                <NuxtLink
                  ...
                  class="text-dimmed hover:text-default border border-default hover:border-accented rounded px-2 py-0.5 text-xs transition-colors"
                  :class="{
                    'border-accented text-default': post.tags?.includes(
                      tag.name,
                    ),
                  }"
                >
